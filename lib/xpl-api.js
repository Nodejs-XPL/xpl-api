...
var APIv1 = function(configuration) {
	...
	this._configuration.xplPort = this._configuration.xplPort || 3865;
	this._configuration.hubSupport = this._configuration.hubSupport || false;
	...
	this._configuration.hubPingDelaySecond = this._configuration.hubPingDelaySecond || 60 * 4;
	this._configuration.xplSource = this._configuration.xplSource || "nodejs."
			+ os.hostname() + "-" + process.pid;
	this._configuration.xplTarget = this._configuration.xplTarget || "*";
...
APIv1.fillCommander = function(commander) {
...
APIv1.prototype._startHub = function(callback) {
	...
	this._getInputBroadcastSocket(function(error, socket, address) {
		...
		if (!self._configuration.hubSupport) {
			...
			self.on("messsage", function(message, address) {
				...
				if (message.bodyName == "hbeat.app") {
					var key = address.address + ":" + address.xplPort;
					var now = Date.now();
					clients[key] = {
						ttl : now + self._configuration.hubPingDelaySecond*1000,
						address : address.address,
						port : address.xplPort
					};
...
APIv1.prototype._forwardMessage = function(clients, message) {
	...
	this._getLocalSocket(function(error, socket) {
		...
		for ( var clientName in clients) {
			...
			socket.send(buffer, 0, buffer.length, client.xplPort, client.address,
					sendMessage);
		}
	});
};
...
APIv1.prototype.sendBufferMessage = function(buffer, callback) {
	...
	this._getOutputBroadcastSocket(function(error, socket) {
		...
		socket
				.send(buffer, 0, buffer.length, self._configuration.xplPort,
...
APIv1.prototype._getInputBroadcastSocket = function(callback) {
	...
	return this._getSocket(this._broadcastInputSocketSemaphore,
			"_inputBroadcastSocket", ba, this._configuration.xplPort, true,
			callback);
};
