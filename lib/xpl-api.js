...
XplAPI.prototype._connect = function(address, port, broadcastType, callback) {
  ...
  socket.on("message", function(buffer, address) {
    ...
    try {
      packet = self._parseXPLMessage(message, address);
...
XplAPI.prototype._parseXPLMessage = function(buffer, address) {
...
XplAPI.prototype.sendXplCommand = function(command, body, bodyName, target,
    callback) {
  ...
  return this._sendXplX(body, bodyName, callback, command, target);
};
...
XplAPI.loadDeviceAliases = function(deviceAliases) {
  var ds = {};
  if (!deviceAliases) {
    return ds;
  }
