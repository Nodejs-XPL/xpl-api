...
APIv1.prototype.bind = function(callback) {
	...
	this._startHub(function(error) {
		if (error) {
			self._log(
					"Start Hub return error (a XPL-HUB is already launched ?)",
					error);
...
APIv1.prototype.sendBufferMessage = function(buffer, callback) {
	...
	this._getOutputBroadcastSocket(function(error, socket) {
		...
		self._log("Send buffer to " + self._configuration.broadcastAddress
...
APIv1.prototype._getOutputBroadcastSocket = function(callback) {
	...
	return this._getSocket(this._broadcastOutputSocketSemaphore,
			"_outputBroadcastSocket", this._configuration.localAddress, 0,
			true, callback);
};
...
APIv1.prototype._getSocket = function(sem, cacheName, address, port,
		broadcastType, callback) {

	var self = this;

	this._log("_getSocket: '" + cacheName + "' get socket for address=",
			address, " port=", port, " broadcast=", broadcastType);
	...
	sem.take(function() {
		...
		self._connect(address, port, broadcastType,
				function(error, socket, address) {
					self._log("_getSocket: '" + cacheName
							+ "' Connection result error=", error, "address=",
							address);
...
APIv1.prototype._connect = function(address, port, broadcastType, callback) {
	...
	this._log("_connect: address=", address, " port=", port, " broadcastType=",
			broadcastType);
	...
	socket.on("listening", function(error) {
		...
		self._log("Bind succeed on ", +address.address + ":" + address.port);
