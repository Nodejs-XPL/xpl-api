...
var APIv1 = module.exports = function(configuration) {
	...
	this.configuration.localAddress = this.configuration.hubAddress
			|| "127.0.0.1";
...
APIv1.prototype._startHub = function(callback) {
	...
	this._connect(this.configuration.broadcastAddress, this.configuration.port,
			false, function(error, emitter, address, socket) {
				...
				console.log("startHub: Hub started ", error, emitter, address,
						socket);
				if (error) {
...
APIv1.prototype._connectHub = function(callback) {
	...
	this._connect(this.configuration.localAddress, 0, false, function(err,
		...
		self._hubInterval = setInterval(function() {
			self._hubPing(emitter, address, socket);

		}, self.configuration.hubPingDelayMs);
		self._hubPing(emitter, address, socket);

		return callback(null, emitter, address);
	});

};

APIv1.prototype._hubPing = function(emitter, address, socket) {
	...
	console.log("hubPing: buffer=", buffer, " port=", this.configuration.port,
			" address=", this.configuration.localAddress);
...
APIv1.prototype._connect = function(address, port, broadcastType, callback) {
	...
	var socket = Dgram.createSocket(config.socketType, function(event, buffer,
			param1, param2) {
		console.log("_connect: Hub socket receives event ", event, " from ",
				param2);
		...
		var address = param1 && param1.address();
		...
		case "message":
			...
			if (packet) {
				emitter.emit("message", packet, address);

				if (packet.headerName) {
					emitter.emit("xpl:" + packet.headerName, packet, address);
				}
				if (packet.bodyName) {
					emitter.emit("xpl:" + packet.bodyName, packet, address);
				}
			}
			break;
		}
