...
var proto = {
	...
	_forwardMessage: function(clients, message) {
		var now = Date.now();
		...
		this._getLocalSocket(function(error, socket) {
			...
			for ( var clientName in clients) {
				...
				socket.send(buffer, 0, buffer.length, client.port, client.address, sendMessage);
			}
		});
	},

	_connectHub: function(callback) {
	...
	_fillHeader: function(message, headerName) {
		message = message || {};
		message.header = {
			hop: 1,
	...
	_hubPing: function(address, socket) {
	...
	sendMessage: function(headerName, header, bodyName, body, callback) {
		var message = {
	...
	_xplMessageToBuffer: function(xplMessage) {
	...
	send: function(xplMessage, callback) {
	...
	sendBufferMessage: function(buffer, callback) {
		...
		this._getOutputBroadcastSocket(function(error, socket) {
			...
			socket.send(buffer, 0, buffer.length, self._configuration.xplPort, self._configuration.broadcastAddress,
	...
	_getInputBroadcastSocket: function(callback) {

		var ba = (os.platform() == "win32") ? "" : this._configuration.broadcastAddress;

		return this._getSocket(this._broadcastInputSocketSemaphore, "_inputBroadcastSocket", ba,
				this._configuration.xplPort, true, callback);
	},

	_getSocket: function(sem, cacheName, address, port, broadcastType, callback) {
	...
	_connect: function(address, port, broadcastType, callback) {
	...
	_parseXPLMessage: function(buffer) {
	...
	_parseXPLBlock: function(dest, blockName, blockVar, lines) {
	...
	_sendXplX: function(body, bodyName, callback, headerName) {
	...
	sendXplStat: function(body, bodyName, callback) {
	...
	sendXplTrig: function(body, bodyName, callback) {
		callback = arguments[arguments.length - 1];
		if (typeof (callback) != "function") {
			callback = null;
		}
	...
	sendXplCmnd: function(body, bodyName, callback) {
		callback = arguments[arguments.length - 1];
		if (typeof (callback) != "function") {
			callback = null;
		}
