...
APIv1.prototype.wait = function(callback) {
	...
	this._startHub(function(error) {
		if (error) {
			console.log("Start Hub return error (a HUB is already started ?",error);
...
APIv1.prototype._connect = function(address, port, broadcastType, callback) {
	...
	console.log("_connect: address=",address," port=",port, " broadcastType=",broadcastType);

	var self = this;
	var socket = Dgram.createSocket(config.socketType, function(event, buffer,
			param1, param2) {
		console.log("_connect: Hub socket receives event ", event, " from ", param2);
	...
	socket.on("listening", function () {
		...
		console.log("_connect: socket listening " + address.address + ":" + address.port);
	});
