...
APIv1.prototype.bind = function(callback) {
	this.close();
...
APIv1.prototype._getSocket = function(sem, cacheName, address, port, broadcastType,
		callback) {

	var self = this;

	this._log("_getSocket: '"+cacheName+"' get socket for address=",address, " port=", port, " broadcast=",broadcastType);
	...
	sem.take(function() {
		...
		self._connect(address, port, broadcastType,
				function(error, socket, address) {
					self._log("_getSocket: '"+cacheName+"' Connection result ",error, address);
...
APIv1.prototype.sendXplCmnd = function(body, bodyName, callback) {
	callback = arguments[arguments.length - 1];
	if (typeof (callback) != "function") {
		callback = null;
	}
